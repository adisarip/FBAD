...
#pragma HLS dependence variable=ans type=inter dependent=false
		...
		area = (x2-x1)*(y2-y1)/2 ;
		ans[j]=(ar[x2%size][y2] - (y1 > 0? ar[x2%size][y1-1]:0) - (x1>0?ar[(x1-1)%size] : 0) + ((y1 > 0) & ( x1>0)?ar[(x1-1)%size][y1-1]:0)) > area? 255 : 0 ;
...
#pragma HLS ARRAY_PARTITION variable=arr type=complete dim=2
		...
		Load_data:for(uint i =0 ; i< size*width ; i++){
#pragma HLS PIPELINE
...
#pragma HLS dataflow
			Transfer_in:for (uint j= 0; j < width; j++ ){
#pragma HLS PIPELINE
#pragma HLS unroll factor=64
